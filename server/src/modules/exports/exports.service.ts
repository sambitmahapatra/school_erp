...
export function exportAttendance(filters: { date?: string; classId?: number; subjectId?: number }) {
  const db = getDb();
...
export function exportMarks(filters: { examId: number; classId: number; subjectId: number }) {
  const db = getDb();
  const rows = db
    .prepare(
...
export function exportLeaves(month?: string) {
  const db = getDb();
  const rows = month
    ? db
        .prepare(
